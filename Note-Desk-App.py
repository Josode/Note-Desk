...
class MyNote(Frame):
    ...
    # icon frame
        ...
        # reminder
        self.reminder_button = Button(self.icon_frame, command=lambda: self.user_input(),
                                    width=25, height=25, bg=self.top_color, bd=0, relief=FLAT,
                                    activebackground=self.top_color)
...
# adds reminder to note
    def reminder_note_event(self, event):
        self.reminder_note(int(self.username_entry.get()))

    def reminder_note(self, time):
        try:
            file = self.file_name
            ...
            def remind_user(seconds, reminder):
                ...
                self.destroy_user_input_window()
                t.start()

            def start(reminder):
                print("\nReminder: \n" + reminder)

                self.popup_window(reminder_content=reminder)

            remind_user(time, contents)

        except AttributeError:
            pass

    # set reminder window
    def user_input(self, label=None, height=None):
        try:
            print("\nSet reminder for: " + self.file_name)

            self.user_input_window = Toplevel()

            self.user_input_window.bind("<Return>", self.reminder_note_event)

            self.user_input_window.configure(bg=textbox_bg)

            # makes only user input window usable
            self.user_input_window.focus_set()
            self.user_input_window.grab_set()

            # sets w/h and position
            self.user_input_window.geometry("180x180+100+100")

            # frame for entry field
            enter_username_frame = Frame(self.user_input_window, bg=textbox_bg)
            enter_username_frame.pack(pady=5)

            if label:
                self.window_label = Label(enter_username_frame, text=label, fg=primary_fg)
                self.window_label.pack(pady=4, padx=4)

            self.username_entry = Entry(enter_username_frame, width=22, bg=textbox_bg, fg=primary_fg, bd=1,
                                        insertbackground=primary_fg)
            self.username_entry.pack(pady=3, padx=10)

            # Frame for Change button and cancel button
            buttons_frame = Frame(self.user_input_window, bg=textbox_bg)
            buttons_frame.pack()

            change_button = Button(buttons_frame, relief=GROOVE, text="Change", width=8, bg=textbox_bg, bd=1,
                                   fg=primary_fg, activebackground=textbox_bg, activeforeground=primary_fg,
                                   command=lambda: self.reminder_note_event(event=None))
            change_button.pack(side=LEFT, padx=4, pady=3)

            # cancel
            cancel_button = Button(buttons_frame, relief=GROOVE, text="Cancel", width=8, bg=textbox_bg, bd=1,
                                   fg=primary_fg, command=self.destroy_user_input_window,
                                   activebackground=textbox_bg, activeforeground=primary_fg)
            cancel_button.pack(side=RIGHT, padx=4, pady=3)

        except AttributeError:
            print("No note opened.")

    def destroy_user_input_window(self):
        self.user_input_window.destroy()

    # final error popup when time
    def popup_window(self, reminder_content, height='100', button_msg="Okay"):

        # makes top level with placement relative to root and specified error msg
        self.popup_window_tl = Toplevel(bg=textbox_bg)
        self.popup_window_tl.focus_set()
        self.popup_window_tl.grab_set()
        ...
        too_long_frame = Frame(self.popup_window_tl, bg=textbox_bg)
        too_long_frame.pack(fill=BOTH, expand=True)

        self.menu_bar_frame = Frame(too_long_frame, heigh="120", bg=self.top_color, bd="10")
        self.menu_bar_frame.pack_propagate(False)
        self.menu_bar_frame.pack(side=TOP, fill=X)
        menu_label = Label(self.menu_bar_frame, text="Reminder", font="Helvetica 26", bg=self.top_color, fg="#000000")
        menu_label.pack(padx=5, pady=5, side=LEFT)

        self.error_scrollbar = Scrollbar(too_long_frame, bd=0)
        self.error_scrollbar.pack(fill=Y, side=RIGHT)

        error_text = Text(too_long_frame, bg=textbox_bg, fg=primary_fg, wrap=WORD, relief=FLAT,
                          yscrollcommand=self.error_scrollbar.set, spacing3=8, height="10")
        error_text.pack(pady=15, padx=15, fill=BOTH)
        error_text.insert(INSERT, reminder_content)
        error_text.configure(state=DISABLED)
        button_frame = Frame(too_long_frame, width=12)
        button_frame.pack()

        okay_button = Button(button_frame, relief=FLAT, bd=0, text=button_msg, bg="#dbdbdb", font="Helvetica 12",
                             fg=primary_fg, activebackground=textbox_bg, height=2, width=10,
                             activeforeground=primary_fg, command=lambda: self.popup_window_tl.destroy())
        okay_button.pack(side=BOTTOM, fill=X, padx=5, pady=10)
